...
async def async_setup_entry(hass, entry: MitsubishiWfRacConfigEntry, async_add_entities):
    ...
    async_add_entities([AircoClimate(device, entry.runtime_data, hass)])
...
class AircoClimate(WfRacEntity, ClimateEntity):
    ...
    def __init__(self, device: Device, data: MitsubishiWfRacData, hass: HomeAssistant) -> None:
    ...
    async def async_set_preset_mode(self, preset_mode):
        ...
        if preset_mode_obj.hvac_mode == HVACMode.OFF:
            ...
            _swing_lr = (
                SWING_HORIZONTAL_MODE_TRANSLATION[SWING_HORIZONTAL_AUTO]
                if _swing_auto
                else SWING_HORIZONTAL_MODE_TRANSLATION[horizontal_swing_mode]
            )
